...
import { Loader2, Scissors, CheckCircle, Search, FilePlus, UserPlus, Clock, User, Phone, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row gap-6 md:items-center justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-primary">{t('shopDashTitle')}</h1>
          <p className="text-muted-foreground mt-2">{t('shopDashSubtitle')}</p>
        </div>
        <CustomerQuickSearch />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Link href="/shop/invoices?status=under_tailoring">
          <DashStatCard title={t('underTailoring')} value={stats?.underTailoring || 0} icon={<Scissors className="w-6 h-6 text-accent" />} color="bg-accent/10" />
        </Link>
        <Link href="/shop/invoices?status=ready">
          <DashStatCard title={t('readyDelivery')} value={stats?.readyForDelivery || 0} icon={<Clock className="w-6 h-6 text-emerald-500" />} color="bg-emerald-500/10" />
        </Link>
        <Link href="/shop/invoices?status=delivered">
          <DashStatCard title={t('deliveredToday')} value={stats?.deliveredToday || 0} icon={<CheckCircle className="w-6 h-6 text-blue-500" />} color="bg-blue-500/10" />
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Link href="/shop/customers" className="block">
            <Card className="hover-elevate cursor-pointer border-0 shadow-md bg-gradient-to-br from-primary to-primary/90 text-primary-foreground rounded-2xl overflow-hidden relative">
              ...
              <CardContent className="p-6 flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-bold text-lg">{t('newInvoice')}</h4>
                  <p className="text-primary-foreground/70 text-sm">{t('newInvoiceDesc')}</p>
                </div>
              </CardContent>
            </Card>
          </Link>
          <Link href="/shop/customers?new=true" className="block">
            <Card className="hover-elevate cursor-pointer border-0 shadow-md bg-white rounded-2xl">
              <CardContent className="p-6 flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-bold text-lg text-foreground">{t('newCustomer')}</h4>
                  <p className="text-muted-foreground text-sm">{t('newCustomerDesc')}</p>
                </div>
              </CardContent>
            </Card>
          </Link>
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/shop/invoices" className="text-sm font-bold text-accent hover:underline">{t('viewAll')}</Link>
          </div>
          <Card className="border-0 shadow-xl rounded-2xl overflow-hidden">
            <div className="divide-y">
                    ...
                    <div className="flex flex-col">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" className={`border-0 text-xs rounded-full ${
                          ...
                          {inv.status === 'ready' ? t('statusReady') : inv.status === 'delivered' ? t('statusDelivered') : t('statusUnder')}
                        </Badge>
                      </div>
                      <span className="text-sm text-muted-foreground mt-1">{inv.customerName} - <span dir="ltr">{inv.customerPhone}</span></span>
                    </div>
                    <div className="text-left">
                      <div className="font-bold text-lg">{inv.totalAmount} {t('kwd')}</div>
              ...
              {(!recentInvoices?.invoices || recentInvoices.invoices.length === 0) && (
                <div className="p-8 text-center text-muted-foreground">{t('noRecentInvoices')}</div>
              )}
            </div>
          </Card>
        </div>
